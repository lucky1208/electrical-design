...
window.SYM = (function () {
  ...
  function schedule(rows, x, y, w, title) {
    ...
    /* v2.7.1-INTEG-C2: 第四列「厂商参考」——可点击的厂商/资料入口链接。
     * 只有通过 https + 公网校验的链接才会渲染成 <a>；列宽固定，
     * 链接文案按 linkKind 如实标注，绝不把厂商首页写成「手册下载」。 */
    const linkW = 76;
    const c3 = x + w - linkW - 4;
    const specWidth = c3 - c2 - 8;
    const prepared = rows.map((row) => ({
      tag: row.tag, name: clip(row.name, 8.5, c2 - c1 - 6),
      lines: wrap(row.spec, 8, specWidth, 3),
      linkLabel: row.referenceUrl ? clip(row.referenceLinkLabel || '产品页', 7.5, linkW - 8) : '',
      linkUrl: row.referenceUrl || ''
    }));
    ...
    prepared.forEach((row, i) => {
      ...
      if (row.linkUrl) {
        s += '<a href="' + esc(row.linkUrl) + '" target="_blank" rel="noopener noreferrer">' +
          '<title>' + esc(row.linkUrl) + '</title>' +
          txt(c3 + 5, cursor + 9, row.linkLabel, 8, C.link || '#2f6fb2', 'start') + '</a>';
      }
      cursor += heights[i];
