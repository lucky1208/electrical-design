...
window.EVSE_ENGINE = (function () {
  ...
  function build(P0) {
    ...
    /* 档位选取：超出内置标称序列时取顶格，并**强制**把「需求 vs 选用」这一事实
     * 记录为结构化数据，使图面、BOM、CLI、审计四处都能看到。
     * v2.7.1-FIX-E1：修复前只 push 一条字符串告警到 warnings，最终只落在巨型
     * JSON 深处，图面与 BOM 仍把顶格值当成正常选型呈现（实测 1024.7A 进线场景
     * 图上照写 800A 接触器，而需求是 1281A）。 */
    const overRange = [];
    const pick = (value, series, label) => {
      if (STD.exceedsSeries(value, series)) {
        const ceiling = series[series.length - 1];
        const required = Math.ceil(value);
        const entry = {
          label,
          required,
          selected: ceiling,
          ceiling,
          deficitA: Math.max(0, required - ceiling),
          message: label + ' 所需容量 ' + required + ' 已超出内置标称档位序列上限 ' + ceiling +
            '，必须由电气专业按厂家型录另行选型。'
        };
        overRange.push(entry);
    ...
    /* v2.7.1-FIX-B1: PJ1 是交流计量点，规格必须取交流电能表文案。
     * 修复前此处与 PJ2 共用 std.meter（直流表文案），导致 BOM 与图面明细表
     * 把“交流电能表”标成“直流电能表（国网型式批准 + 强检）”，询价会买错表。 */
    addSched('PJ1', '交流电能表', (std.acMeter || std.meter) + '·母线取电');
    ...
    const readiness = {
      ...
      summary: { assumptionCount: assumptions.length, warningCount: warnings.length, overRangeCount: overRange.length },
      entries: [],
      blockingItems: [],
      release: {
    ...
    const baseResult = {
      ...
      assumptions, warnings, overRange, readiness, releaseGate: readiness.release, design,
