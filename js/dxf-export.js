/* ============================================================
 * AIDC SVG → DXF concept exporter
 * ------------------------------------------------------------
 * Creates an editable R2010 DXF concept sketch from the SVG preview.
 * It exports only primitive geometry and the AIDC CAD layer manifest.
...
window.AIDC_DXF = (function () {
  ...
  function layerFor(node) {
    ...
    if (node.tagName && node.tagName.toLowerCase() === 'text') return 'AIDC-TEXT';
    const color = (inherited(node, 'stroke') || inherited(node, 'fill') || '').toLowerCase();
    const dashed = Boolean(inherited(node, 'stroke-dasharray'));
    if (color === '#2563eb' || color === '#1d4ed8') return 'AIDC-MV';
    if (color === '#16a34a' || color === '#15803d') return 'AIDC-LV';
    if (color === '#7c3aed' || color === '#6d28d9') return 'AIDC-UPS';
    if (color === '#9333ea' && dashed) return 'AIDC-GEN';
    if (color === '#b45309' && dashed) return 'AIDC-BAT';
    if (color === '#475569' && dashed) return 'AIDC-CTL';
    if (color === '#64748b') return 'AIDC-ANNO';
    if (color === '#dc2626') return 'AIDC-COOL-RET';
    if (color === '#059669' || color === '#0f766e') return 'AIDC-COOL-COND';
    if (color === '#0284c7' || color === '#0ea5e9' || color === '#4ac8ff') return 'AIDC-COOL-SUP';
    return 'AIDC-EQPT';
  }
  ...
  function exportSvg(svgMarkup, options) {
    ...
    const comment = cleanText((options && options.comment) || 'AIDC 可编辑 DXF 方案级草图；复杂符号、尺寸、保护与设备型号须由项目 CAD 模板和专业人员复核。');
    const manifestData = {
      schema: 'AIDC-DXF-MANIFEST/1.0', output: 'DXF_R2010_CONCEPT', units: 'mm', paperMm: [paperW, paperH], document: documentControl,
      layers: cloneManifest(manifest),
      scope: {
        included: ['SVG primitive geometry', 'AIDC layer table', 'text annotations'],
  ...
  return { exportSvg, fromSvg: exportSvg, layerManifest: () => cloneManifest(FALLBACK_LAYER_MANIFEST), MANIFEST_SCHEMA: 'AIDC-DXF-MANIFEST/1.0' };
})();
